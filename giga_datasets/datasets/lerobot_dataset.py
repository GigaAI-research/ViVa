...
class LeRobotDataset(BaseDataset):
    ...
    def __init__(
        ...
        default_advantage: int = 0,
        **kwargs,
    ) -> None:
        """Initialize LeRobotDataset with optional advantage information.
        
        Args:
            data_path: Path to the dataset.
            data_size: Optional size of the dataset.
            delta_info: Delta timestamp information.
            meta_name: Name for attaching metadata.
            default_advantage: Default advantage value if not found in data (default: 0).
                              Set to 0 (negative) or 1 (positive).
    ...
    def _get_data(self, index: int) -> dict:
        ...
        # Read advantage_indicator from parquet data (frame-level)
        # If 'advantage_indicator' exists in the data, use it
        # Otherwise, use default_advantage
        if 'advantage_indicator' in data_dict:
            # Convert to standard Python int
            advantage = data_dict['advantage_indicator']
            if isinstance(advantage, torch.Tensor):
                data_dict['advantage_indicator'] = int(advantage.item())
            elif isinstance(advantage, np.ndarray):
                data_dict['advantage_indicator'] = int(advantage)
            else:
                data_dict['advantage_indicator'] = int(advantage)
        else:
            # Use default value if not present
            data_dict['advantage_indicator'] = self.default_advantage
